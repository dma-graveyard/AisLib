...
public class PastTrack implements Serializable {
	...
	private NavigableSet<PastTrackPoint> points = new TreeSet<PastTrackPoint>();
	...
	public synchronized void addPosition(AisVesselPosition vesselPosition, int minDist) {
		...
		GeoLocation pointPos = new GeoLocation(newPoint.getLat(), newPoint.getLon());
		if (prevPoint != null) {
			GeoLocation prevPos = new GeoLocation(prevPoint.getLat(), prevPoint.getLon());
			if (prevPos.getRhumbLineDistance(pointPos) < minDist) {
				points.remove(prevPoint);
			}
		}
		if (nextPoint != null) {
			GeoLocation nextPos = new GeoLocation(nextPoint.getLat(), nextPoint.getLon());
			if (nextPos.getRhumbLineDistance(pointPos) < minDist) {
...
//		}
	...
	public synchronized List<PastTrackPoint> getPoints() {
		List<PastTrackPoint> list = new ArrayList<PastTrackPoint>();
