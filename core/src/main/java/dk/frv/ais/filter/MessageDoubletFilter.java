 ...
 * A doublet filter.
 * 
 * The doublet filter works by only allowing the same message through once in a
 * time window. The six bit string of the message is used as unique identifier.
 * 
 */
public class MessageDoubletFilter extends GenericFilter {
	 ...
	 * is rather effective for differing messages. Using HashMap can result in
	 * wrong filtering as two strings can produce the same hash code.
	 */
