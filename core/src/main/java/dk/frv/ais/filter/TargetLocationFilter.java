...
import dk.frv.ais.message.IPositionMessage;

/**
 * Simple filtering based on the location of targets.
 * Filtered on a list of geometries. 
 */
public class TargetLocationFilter extends GenericFilter {
	 ...
	 */
	private Map<Long, GeoLocation> posMap = new ConcurrentHashMap<Long, GeoLocation>();
	
	/**
	 * List of geometries 
	 */
	private List<FilterGeometry> geomtries = new ArrayList<FilterGeometry>();
...
//
		// Get location
		GeoLocation loc = posMap.get(aisMessage.getUserId());
		if (loc == null) {
			return;
		}
