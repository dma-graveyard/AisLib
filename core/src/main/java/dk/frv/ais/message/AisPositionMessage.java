 ...
 * 
 * An AIS position message is defined by ITU-R M.1371-4 in annex 8 - AIS
 * messages section 3.1
 * 
 * This is a generalization for message types 1-3 and possibly more in the
 * future.
 * 
 * Type 1: Scheduled position report (Class A shipborne mobile equipment) Type
 * 2: Assigned scheduled position report (Class A shipborne mobile equipment)
 * Type 3: Special position report, response to interrogation;(Class A shipborne
 * mobile equipment)
 * 
 */
public abstract class AisPositionMessage extends AisMessage implements IGeneralPositionMessage {
	 ...
	 * under command 3 = restricted manoeuvrability 4 = constrained by her
	 * draught 5 = moored 6 = aground 7 = engaged in fishing 8 = under way
	 * sailing 9 = reserved for future amendment of navigational status for
	 * ships carrying DG, HS, or MP, or IMO hazard or pollutant category C, high
	 * speed craft (HSC), 10 = reserved for future amendment of navigational
	 * status for ships carrying dangerous goods (DG), harmful substances (HS)
	 * or marine pollutants (MP), or IMO hazard or pollutant category A, wing in
	 * grand (WIG); 11-13 = reserved for future use 14 = AIS-SART (active) 15 =
	 * not defined = default (also used by AIS-SART under test)
	 */
	protected int navStatus; // 4 bits

	/**
	 * Rate of Turn (ROT): 0 to +126 = turning right at up to 708 degrees per
	 * min or higher 0 to -126 = turning left at up to 708 degrees per min or
	 * higher Values between 0 and 708 degrees per min coded by ROTais = 4.733
	 * SQRT(ROTsensor) degrees per min where ROTsensor is the Rate of Turn as
	 * input by an external Rate of Turn Indicator (TI). ROTais is rounded to
	...
	/**
	 * Special manoeuvre indicator: 0 = not available = default 1 = not engaged
	 * in special manoeuvre 2 = engaged in special manoeuvre (i.e.: regional
	 * passing arrangement on Inland Waterway)
	 * 
	...
	public boolean isPositionValid() {
		GeoLocation geo = pos.getGeoLocation();
